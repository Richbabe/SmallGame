                              ...
                              )  
{
    ...
    {
        V_RETURN( CompileShaderFromFile( L"RenderVarianceShadow.hlsl", "VSMain", m_cvsModel, &m_pvsRenderVarianceShadowBlob ) );
    }
    ...
    if ( m_ppsRenderVarianceShadowBlob == NULL ) 
    {
        V_RETURN( CompileShaderFromFile( L"RenderVarianceShadow.hlsl", "PSMain", m_cpsModel, &m_ppsRenderVarianceShadowBlob ) );
    }
    ...
    {
        V_RETURN( CompileShaderFromFile( L"2DQuadShaders.hlsl", "VSMain", m_cvsModel, &m_pvsQuadBlurBlob ) );
    }
    ...
    for ( INT iBlurKernelIndex = 0; iBlurKernelIndex < MAXIMUM_BLUR_LEVELS; ++iBlurKernelIndex ) {
        ...
        if ( m_ppsQuadBlurXBlob[iBlurKernelIndex] == NULL )
        {
            V_RETURN( CompileShaderFromFile( 
                L"2DQuadShaders.hlsl", "PSBlurX", m_cpsModel, &m_ppsQuadBlurXBlob[iBlurKernelIndex], kernelDefines ) );
        }
        ...
        {
            V_RETURN( CompileShaderFromFile( 
                L"2DQuadShaders.hlsl", "PSBlurY", m_cpsModel, &m_ppsQuadBlurYBlob[iBlurKernelIndex], kernelDefines ) );
        }
    ...
    for( INT iCascadeIndex=0; iCascadeIndex < MAX_CASCADES; ++iCascadeIndex ) 
    {
        ...
        {
            V_RETURN( CompileShaderFromFile( L"RenderVarianceScene.hlsl", "VSMain", 
                m_cvsModel, &m_pvsRenderSceneBlob[iCascadeIndex], defines ) );
        }
        ...
        for( INT iBlendIndex=0; iBlendIndex < 2; ++iBlendIndex ) 
        {
            for( INT iIntervalIndex=0; iIntervalIndex < 2; ++iIntervalIndex ) 
            {
                ...
                {
                    V_RETURN( CompileShaderFromFile( L"RenderVarianceScene.hlsl", "PSMain", 
                        m_cpsModel, 
