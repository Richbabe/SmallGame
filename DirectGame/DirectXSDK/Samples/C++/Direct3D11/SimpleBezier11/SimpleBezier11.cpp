...
//--------------------------------------------------------------------------------------
HRESULT CompileShaderFromFile( WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut, const D3D10_SHADER_MACRO* pDefines = NULL )
{
	HRESULT hr = S_OK;
    ...
    hr = D3DX11CompileFromFile( str, pDefines, NULL, szEntryPoint, szShaderModel, D3D10_SHADER_ENABLE_STRICTNESS, NULL, NULL, ppBlobOut, &pErrorBlob, NULL );
    if( FAILED(hr) )
    {
        if( pErrorBlob )
                                      ...
                                      void* pUserContext )
{
    ...
    V_RETURN( CompileShaderFromFile( L"SimpleBezier11.hlsl", "BezierVS",     "vs_5_0", &pBlobVS ) );
    V_RETURN( CompileShaderFromFile( L"SimpleBezier11.hlsl", "BezierHS",     "hs_5_0", &pBlobHSInt, integerPartitioning ) );
    V_RETURN( CompileShaderFromFile( L"SimpleBezier11.hlsl", "BezierHS",     "hs_5_0", &pBlobHSFracEven, fracEvenPartitioning ) );
    V_RETURN( CompileShaderFromFile( L"SimpleBezier11.hlsl", "BezierHS",     "hs_5_0", &pBlobHSFracOdd, fracOddPartitioning ) );
    V_RETURN( CompileShaderFromFile( L"SimpleBezier11.hlsl", "BezierDS",     "ds_5_0", &pBlobDS ) );
    V_RETURN( CompileShaderFromFile( L"SimpleBezier11.hlsl", "BezierPS",     "ps_5_0", &pBlobPS ) );
    V_RETURN( CompileShaderFromFile( L"SimpleBezier11.hlsl", "SolidColorPS", "ps_5_0", &pBlobPSSolid ) );
