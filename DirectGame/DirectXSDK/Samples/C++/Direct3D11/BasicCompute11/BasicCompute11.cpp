...
//
// Demonstrates the basics to get DirectX 11 Compute Shader up and running 
// by implemeting Array A + Array B
//
...
//--------------------------------------------------------------------------------------
HRESULT CreateComputeDevice( ID3D11Device** ppDeviceOut, ID3D11DeviceContext** ppContextOut, BOOL bForceRef = FALSE );
...
HRESULT CreateRawBufferOnGPU( ID3D11Device* pDevice, UINT uSize, VOID* pInitData, ID3D11Buffer** ppBufOut );
...
int __cdecl main()
{
    ...
    if ( FAILED( CreateComputeDevice( &g_pDevice, &g_pContext/*, TRUE*/ ) ) )
        return 1;
    printf( "done\n" );

    printf( "Creating Compute Shader..." );
    CreateComputeShader( L"BasicCompute11.hlsl", "CSMain", "cs_4_0", g_pDevice, &g_pCS );
    printf( "done\n" );
    ...
    // Create structured buffer on the GPU
    CreateStructuredBufferOnGPU( g_pDevice, sizeof(BufType), (UINT)g_vBuf0.size(), &g_vBuf0[0], &g_pBuf0 );
    CreateStructuredBufferOnGPU( g_pDevice, sizeof(BufType), (UINT)g_vBuf1.size(), &g_vBuf1[0], &g_pBuf1 );
    CreateStructuredBufferOnGPU( g_pDevice, sizeof(BufType), (UINT)g_vBuf0.size(), NULL, &g_pBufResult );
    
    // You can also play with the raw buffer type instead, you have to change the hlsl shader code to match this,
    // See BasicCompute11.hlsl
    /*CreateRawBufferOnGPU( g_pDevice, (UINT)g_vBuf0.size()*sizeof(BufType), &g_vBuf0[0], &g_pBuf0 );
    CreateRawBufferOnGPU( g_pDevice, (UINT)g_vBuf1.size()*sizeof(BufType), &g_vBuf1[0], &g_pBuf1 );
    CreateRawBufferOnGPU( g_pDevice, (UINT)g_vBuf0.size()*sizeof(BufType), NULL, &g_pBufResult );*/
    printf( "done\n" );
